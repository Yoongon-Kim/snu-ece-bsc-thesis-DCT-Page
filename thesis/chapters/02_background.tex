...
the order of a thousand or more, prefill's intensity sits above the
$\approx 200$--$400$~FLOP/byte ridge of modern GPUs (e.g.\ the A6000
used in this thesis) while decode sits far below it; prefill is
therefore in the compute-bound regime and modifying it offers little
speedup, while decode is squarely memory-bound.  Sparse-attention
...
$q[c] \cdot k[c]$ that requires only the per-channel sign of $q[c]$.
