...
%
% Analytical prose to add after Table 5.4 (per-stage profile):
%   - Bookkeeping (proxy + score + topk/assemble) totals 0.76 ms,
%     less than DCT-Page's own attention kernel (0.63 ms) and an
%     order of magnitude less than Full-KV's 6.4 ms. The speedup
%     therefore comes overwhelmingly from compressing the KV that
%     attention actually consumes, not from the bookkeeping itself
%     being free.
%   - Proxy update is essentially free (0.3% of decode-step
%     attention budget) thanks to the incremental update of
%     Section 3.3.
...
%   - Throughput @ 32K B1 (eager mode for fairness, since Multipole
%     cannot be captured into a CUDA graph): DCT-Page 26.9 tok/s vs
%     Multipole 7.50 tok/s => DCT-Page is 3.59x faster.
...
\begin{table}[ht]
  ...
  {DCT-Page vs.\ Multipole Attention.  RULER and LongBench v1 are
   ...
   generation) and cannot be captured into a CUDA graph, so for
   apples-to-apples comparison both methods are measured in eager
   mode here.  The CUDA-graph throughput of DCT-Page is reported
   in Section~\ref{sec:results-speed}.}
  \label{tab:multipole}
  \footnotesize
  \setlength{\tabcolsep}{5pt}
...
% Analytical prose to add after Table 5.6:
%   - Throughput is essentially flat across P=32..128 (39.15-39.37
%     tok/s, < 1% spread). P=16 pays a ~5% bookkeeping penalty from
%     doubling the page count. The takeaway: page granularity is a
%     selection-fidelity knob, not a throughput knob. Hardware cost
%     is insensitive in the regime we care about.
