...
\begin{figure}[ht]
  ...
  Section~\ref{sec:proxy} (blue) already routes $\approx 84\%$ of
  the dense softmax mass to its selected pages at the most
  aggressive setting $c{=}1$ (one proxy token per $16$ keys), and
  the curve climbs gently to $\approx 90\%$ at $c{=}16$ before
  saturating; the per-channel min/max scorer of Quest (orange)
  covers only $65.6\%$ of the mass at the same selected-token
  budget, regardless of $c$.  Shaded bands are $\pm 1\sigma$ across
  transformer layers.}
...
already attains a paged mass-recall of $\approx 84\%$ averaged across
layers, while Quest's per-channel min/max statistics cover only
$65.6\%$ of the dense softmax mass at the same selected-token budget.
This is an $\approx 18$-point gap in the only intermediate quantity
that connects scorer geometry to downstream attention error, and it
holds before DCT-Page is given any of its bin-budget headroom: as $c$
grows toward $P$, the DCT curve climbs to $\approx 90\%$, widening
the gap to Quest by another $\approx 6$ points while Quest's line
does not move.  The DCT-Page curve is also remarkably flat once it
...
DCT-Page misses $\approx 16\%$ of that mass at $c{=}1$; Quest misses
$\approx 34\%$.  This gap is not academic: it is the mechanism behind
...
distribution is concentrated, missing $34\%$ of its mass is much more
likely to also miss the needle page entirely than missing $16\%$,
