...
$F \in \mathbb{R}^{P \times P}$ be the orthonormal DCT-II matrix from
Eq.~\eqref{eq:dct} and let $\mathrm{trunc}_C(\cdot)$ project onto its
first $C$ rows.  The DCT-lowpass-IDCT page proxy is
...
\end{equation}
i.e.\ apply DCT, keep the leading $C$ coefficients, invert, and
rescale by $\sqrt{C/P}$ so that the proxy preserves the
energy of the retained band.  Crucially $M$ depends only on $(P, C)$,
so it is computed once at start-up and cached as a single
$C \times P$ matrix; at runtime, proxy construction is a single
...
FlashInfer's variable-length paged-attention primitive.  The primitive
accepts a per-(virtual batch, kv-head) index list and computes
ordinary scaled dot-product attention against exactly those pages,
without materializing the unselected KV.  Conceptually, the runtime
treats each $(b, h)$ pair as a ``virtual batch'' with its own page
list, which is how DCT-Page realizes per-kv-head heterogeneous
selection at hardware speed.
